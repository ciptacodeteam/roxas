...
export function PhoneInputAuth({
  ...
  return (
    <div className="bg-foreground ring-offset-background focus-within:ring-ring flex h-10 w-full items-center rounded-md focus-within:ring-2 focus-within:ring-offset-2">
      {/* Country Selector */}
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className="flex h-full items-center gap-2 px-3 text-sm whitespace-nowrap text-gray-400"
          >
        ...
        <PopoverContent
          ...
          align="start"
          sideOffset={8}
          className="bg-foreground w-[280px] border-gray-700 p-0"
        >
          <Command className="bg-foreground border-0 text-white">
            <CommandInput
            ...
            <CommandList className="max-h-[200px]">
              ...
              {countries.map((country) => (
                <CommandItem
                  ...
                  className="flex cursor-pointer items-center gap-3 text-white hover:bg-white/10"
                >
                  <span className="text-lg">{country.flag}</span>
                  <span className="flex-1">{country.name}</span>
                  <span className="text-sm text-gray-200">{country.dial}</span>
                </CommandItem>
              ))}
            </CommandList>
          </Command>
      ...
      {/* Separator Line */}
      <div className="bg-input h-4 w-px" />
      ...
      <Input
        ...
        className="h-full w-full border-0 bg-transparent px-3 py-2 text-sm text-white file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-gray-400 focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50"
      />
    </div>
  );
}
